...
def _generate_heritability_pipeline_req(task: HeritabilityTask):
    ...
    action = Action(always_run=False, block_external_network=False, commands=COMMANDS, container_name=container_name,
                    disable_image_prefetch=False, disable_standard_error_capture=False, enable_fuse=False,
                    environment=environment, ignore_exit_status=False, image_uri=image_uri, publish_exposed_ports=False,
                    run_in_background=False, timeout=TIMEOUT)
