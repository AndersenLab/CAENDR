...
def query_list_datatable(user_filter=None, status_filter=None):
  ...
  total_records = db.session.scalar(select(func.count()).select_from(query.subquery()))
  ...
  with rollback_on_error_handler():
    data = db.session.execute(query.offset(start).limit(length).\
      join(PhenotypeMetadata.phenotype_values)).scalars().all()

  # Count how many rows matched the filters
  filtered_records = db.session.scalar(select(func.count()).select_from(query.subquery()))
