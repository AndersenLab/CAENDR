...
from base.utils.api import jsonify_request
...
def get_isotype_image_urls(isotype_name):
  ''' Returns a list of public urls for images of the isotype in cloud storage '''
  isotype_photo_path = f'{photos_path_prefix}/{isotype_name}'
  image_list = get_blob_list(photos_bucket, isotype_photo_path)
  blob_list = [generate_blob_url(x) for x in image_list]
  return blob_list
