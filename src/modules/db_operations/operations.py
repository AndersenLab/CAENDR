...
def execute_operation(app, db, DB_OP):
    ...
    logger.info(f'WORMBASE_VERSION:{WORMBASE_VERSION}')
    logger.info(f'STRAIN_VARIANT_ANNOTATION_VERSION:{STRAIN_VARIANT_ANNOTATION_VERSION}')
    ...
    if DB_OP == 'DROP_AND_POPULATE_ALL_TABLES':
        ...
        if not result:
            raise Exception(f"DB Connection error: {message}")

    elif DB_OP == 'TEST_MOCK_DATA':
        test_mock_data(app, db, WORMBASE_VERSION, STRAIN_VARIANT_ANNOTATION_VERSION)
        return
...
def test_mock_data(app, db, wb_ver: str, sva_ver: str):
    os.environ["USE_MOCK_DATA"] = "1"
    os.environ["MODULE_DB_OPERATIONS_CONNECTION_TYPE"] = "memory"
    logger.info("Using MOCK DATA")
    drop_and_populate_all_tables(app, db, wb_ver, sva_ver)


def drop_and_populate_all_tables(app, db, wb_ver: str, sva_ver: str):
    if not wb_ver or not sva_ver:
        raise EnvVarError()
    logger.info("Dropping and populating all tables")
    logger.info('WORMBASE_VERSION: {wb_ver} STRAIN_VARIANT_ANNOTATION_VERSION: {sva_ver}')
